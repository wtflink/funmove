...
def order(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			order.key_expires = datetime.datetime.now() + datetime.timedelta(2)
			order.save()
			...
			msg = EmailMultiAlternatives('ur order', text_content, 'whatever@gmail.com', [sendto])
			msg.send()
...
def confirm(request, confirmation_key):
	...
	if order.key_expires < datetime.datetime.now():
